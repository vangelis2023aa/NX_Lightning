...
final class Ryujinx {
    ...
    static func createAccount(name: String, image: Data) {
        // An empty Data has a nil baseAddress, so the old force-unwrap crashed on
        // an empty avatar. Substitute a 1-byte buffer to keep the pointer valid
        // while still passing the real (zero) length.
        let bytes = image.isEmpty ? Data([0]) : image
        let length = Int32(image.count)

        name.withCString { namePtr in
            bytes.withUnsafeBytes { bufferpointer in
                guard let base = bufferpointer.baseAddress else { return }

                create_account(namePtr, base.assumingMemoryBound(to: UInt8.self), length)
            }
        }
    }
